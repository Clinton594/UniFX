					...
					if (forgotbutton.length !== 0) {
						forgotbutton.attr({ href: "javascript:;" }).click(function () {
							...
							if (user_identifier) {
								...
								$.post(`${site.process}forgot-password`, Forgot, function (res) {//after sending email
...
$("input.send-code[data-code]").parent().css({ position: "relative" }).append(
	$("<button>").attr({ type: "button" }).css({ position: "absolute", right: "1px", bottom: "1px", height: "100%", border: 0, padding: "0 15px", fontWeight: "bold", borderTopRightRadius: "3px", borderBottomRightRadius: "3px", fontSize: "small", zIndex: 4 }).text("Send Code").click(function () {
		let button = $(this);
...
$.fn.longClick = function (callback) {
	...
	$(this).mousedown(function (e) {
		timer = setTimeout(function () { e.stopPropagation(); callback(e); }, timeout);
		return false;
	}).mouseup(function (e) {
		e.stopPropagation();
		clearTimeout(timer);
